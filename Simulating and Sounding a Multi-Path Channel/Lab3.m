...
% % ----------------섹션1. 채널모델 비주얼라이징 하기----------------------
% 5g tool box는 matlab 온라인에서 함 pluto부분은 matlab에서함
% %일단 다음채널들을 사용하라
...
% plot(t,abs(yimp))
% legend('x 임펄스 function','경로지연 모델을 통과한 후 임펄스 응답 크기')
% figure(4);
% stem(t,ypow,'BaseValue',-80)
% ylabel('dB');
% title('ximp를 통과시켜 채널이 입력 신호를 어느 시간 지점에서 얼마나 강하게 통과시키는가"');
...
% title('단순 나눗셈으로 주파수영역에서 채널 응답함수 추정(넓은주파수대역신호)')
% xlabel('MHz')
% hpow=pow2db(max(abs(h).^2,1e-8));
% tns=(1:length(hpow))/fsamp;
% figure(6);
% plot(tns,hpow);
% title('IFFT를 이용, 시간영역 채널응답함수(즉,imp함수추정이다)')
% ylabel('dB')
% xlabel('seconds')
% 
...
% 노말라이즈 하면 값의 크기가 다르긴한데 전예제와같이 실제gain을 비교할것이 아니니깐 하는거 같음
hpow=pow2db(abs(h).^2);
...
xlabel('microSeconds');
ylabel('dB')
title('실제 h채널 추정 임펄스응답함수') %왜 dB값이 그냥int값이나 single값이나 정규화값이나 차이가없지?..

classdef  WLANDetect <  matlab.System
    properties
        % Packet properties
    ...
    methods
        ...
        function detectSTF(obj, r)
            ...
            % 
              ...
              obj.rhoSTF=abs(rprodAvg).^2 ./ max(rsqAvg.*rsq2Avg, tol); % 고정지연 상관값 이론식 왜?.. 몰라 (배운적없음)
            % 원래 : obj.rhoSTF=abs(rprodAvg).^2 ./ (rsqAvg+tol) ./ (rsq2Avg + tol);
            % 비슷? 수정 : obj.rhoSTF=abs(rprodAvg).^2 ./ max(rsqAvg.*rsq2Avg, tol); 

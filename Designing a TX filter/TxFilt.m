    ...
    methods (Access = protected)
        function y = stepImpl(obj, x)
            ...
            if obj.backoffAutoScale
                ...
                %
                pwr_y0=mean(abs(y0).^2) %yo 의 전력을 계산하고
                target_pwr=2*obj.txFS^2/db2pow(obj.backoffLev) % 정의임 log(최대전력/신호전력)=backoff
                scale=sqrt(target_pwr/pwr_y0);
                y1=scale*y0;
                ...
                %    y = ...
                 y = complex( ...
        min(max(real(y1), -obj.txFS), obj.txFS), ...
        min(max(imag(y1), -obj.txFS), obj.txFS) );  %클리핑 method...
